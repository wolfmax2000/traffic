...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pushes.update", [$push->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <input type="hidden" name="template_id" value="{{$push->template_id}}" />
            <div class="form-group">
                <label class="required" for="title">Заголовок</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $push->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('desc') ? 'is-invalid' : '' }}" type="text" name="desc" id="desc" value="{{ old('desc', $push->desc) }}" required>
                @if($errors->has('desc'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" type="text" name="url" id="url" value="{{ old('url', $push->url) }}" required>
                @if($errors->has('url'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('geo') ? 'is-invalid' : '' }}" name="geo" id="geo" required>
                    ...
                    @foreach($countries as $cat)
                        <option value="{{ $cat }}" {{ $cat === old('geo', $push->geo) ? 'selected' : '' }}>{{ $cat }}</option>
                    @endforeach
                </select>
                @if($errors->has('geo'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('device') ? 'is-invalid' : '' }}" name="device" id="device">
                    @foreach($devices as $id => $d)
                        <option value="{{ $id }}" {{ $id == old('device', $push->device) ? 'selected' : '' }}>{{ $d }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('top_type') ? 'is-invalid' : '' }}" name="top_type" id="top_type">
                    @foreach($top_types as $id => $d)
                        <option value="{{ $id }}" {{ $id == old('top_type', $push->top_type) ? 'selected' : '' }}>{{ $d }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('domen') ? 'is-invalid' : '' }}" name="domen" id="domen">
                    @foreach($domains as $id => $d)
                        <option value="{{ $d }}" {{ $d == old('domen', $push->domen) ? 'selected' : '' }}>{{ $d }}</option>
                    @endforeach
                </select>
                @if($errors->has('domen'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('push_type') ? 'is-invalid' : '' }}" name="push_type" id="push_type">
                    @foreach($push_types as $id => $d)
                        <option value="{{ $id }}" {{ $id == old('push_type', $push->push_type) ? 'selected' : '' }}>{{ $d }}</option>
                    @endforeach
                </select>
