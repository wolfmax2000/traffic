    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sources.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('utms') ? 'is-invalid' : '' }}" type="text" name="utms" id="utms" value="{{ old('utms', '') }}" required>
                @if($errors->has('utms'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('scripts') ? 'is-invalid' : '' }}" name="scripts[]" id="scripts" multiple>
                    @foreach($scripts as $id => $scripts)
                        <option value="{{ $id }}" {{ in_array($id, old('scripts', [])) ? 'selected' : '' }}>{{ $scripts }}</option>
                    @endforeach
                </select>
