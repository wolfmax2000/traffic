<template>
    <div>
    <div class="items" >
        <div v-for="(item, index) in items" class="item" v-bind:class="{'double': isDouble(index)}" :key="index">
            <a v-bind:href="item.href" target="_blank" class="" v-bind:style="{
                background: 'rgb(' + item.rgb + ')',
                color: 'white'
            }">
                <div class="card_col_1">
                    <img class="card-img" v-bind:src="'https://informerspro.ru/' + item.image" width="312px" />
                </div>
